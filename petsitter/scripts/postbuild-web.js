 ...
 *   - <head>: manifest link, theme-color, apple-touch-icon, iOS standalone
 *     metas, and a description meta (only when the export didn't emit one).
...
const MARKER = '<!-- pawstructions:pwa -->';
...
const headTags = [
  ...
  '<meta name="apple-mobile-web-app-title" content="Pawstructions">',
];

// Only add a description if the export didn't already emit one.
if (!/<meta[^>]*\bname=["']description["']/i.test(html)) {
  headTags.push(
    '<meta name="description" content="Pet care instructions your sitter will love.">'
  );
}
